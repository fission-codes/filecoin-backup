...
export const addressStringToBytes = (address: string): Uint8Array => {
  ...
  if(!address) throw new Error('empty string is invalid address');
  if (address.length < 3) throw new Error('address is too short to parse');
  if (address[0] !== 'f' && address[0] !== 't') {
    throw new Error('unknown network set in address');
  }
  ...
  switch (protocol) {
    case Protocol.ID: {
      // TODO: in glifio address length is max 22; in zondax max 18; double check
      if (address.length > 18) throw new Error('Invalid ID address length');
    ...
    case Protocol.SECP256K1: {
      ...
      if (payload.byteLength !== 20) throw new Error('invalid payload length in secp256k1 address');
      break;
    }
    case Protocol.ACTOR: {
      address_decoded = base32Decode(address.slice(2).toUpperCase(), 'RFC4648');
      ...
      if (payload.byteLength !== 32) throw new Error('invalid payload length in Actor address');
      break;
    }
    case Protocol.BLS: {
      address_decoded = base32Decode(address.slice(2).toUpperCase(), 'RFC4648');
      payload = address_decoded.slice(0, -4);
      checksum = Buffer.from(address_decoded.slice(-4));
      if  (payload.byteLength !== 48) throw new Error('invalid payload length in BLS address');
      break;
    }
    default: {
      throw new Error('invalid address procotol byte');
    }
  }
